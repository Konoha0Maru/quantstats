...
def plot_eoy_returns(returns, title):
    ...
    ax.set_xticklabels(returns.index.year, rotation=45, ha="right")
    plt.tight_layout()
    return plot_to_base64(fig)
...
def plot_worst_5_drawdown_periods(returns, title):
    ...
    returns.to_csv(title + ".txt", header=True)
    fig, ax = plt.subplots(figsize=(16, 8))
    # drawdown_periods = (
...
def get_excess_return_stats(file_path):
    ...
    field_names = ['return_next_1m', 'return_next_1m', 'return_next_1m', 'return_next_8m', 'return_next_6m']
...
for strategy, returns in strategies_data.items():
    html_template += f'<div class="chart"><h2>{strategy} Daily Active Returns</h2><img src="data:image/png;base64,{plot_daily_active_returns(returns, strategy + "Daily Active Returns")}" alt="{strategy} Daily Active Returns"></div>'
